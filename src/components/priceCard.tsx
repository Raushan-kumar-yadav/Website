...
const heroVariant = {
    start: {
        opacity: 1, // Keep content visible immediately
        scale: 0.95
    },
    end: {
        opacity: 1,
...
const heroChildVariant = {
    start: {
        y: 20,
        opacity: 0.8, // Start with partial opacity, not invisible
        scale: 0.98
    },
    end: {
        y: 0,
...
const PriceCard = ({
  ...
  const [isAnimated, setIsAnimated] = useState(true); // Start as true
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Ensure content is always visible, just enhance with animation
  useEffect(() => {
  ...
  return (
    <motion.div 
      ...
      initial="start"
      animate="end" // Always animate to end state
    ...
    >
      ...
      >
        ...
        {/* Card Content - Always visible with subtle animation */}
        <div className="relative z-20 text-white">
          {/* Header Section */}
          <motion.div variants={heroChildVariant} className="mb-4">
            <div className="flex items-center gap-2">
              <h3 className={`font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent flex-shrink-0 ${isMostPopular
                ? 'text-3xl' : 'text-2xl'
                }`}>
                {title}
              </h3>
          ...
          {/* Description */}
          <motion.p variants={heroChildVariant} className={`text-gray-300 mb-8 leading-relaxed break-words ${isMostPopular ? 'text-base' : 'text-sm'
            }`}>
            {desc}
          </motion.p>

          {/* Dotted Divider */}
          <motion.div variants={heroChildVariant} className="mb-8">
          ...
          {/* Features */}
          <motion.div variants={heroChildVariant} className="mb-5 space-y-2 mt-5">
          ...
          {/* Trust indicators */}
          <motion.div variants={heroChildVariant} className="flex justify-center items-center gap-4 mt-6 text-gray-400 flex-wrap">
